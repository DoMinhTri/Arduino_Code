...
String JScriptTrangWifiConnect()
{
    String JVScript = "<script>  function GetWifiName(_Value){ document.getElementById('WifiName').value = _Value; } function ShowPass(){ document.getElementById('password').type   = 'text'; }  function HiddenPass(){ document.getElementById('password').type = 'password'; }\ 
                        function WifiSetup(){ var _Name = document.getElementById('WifiName').value; var _Pass = document.getElementById('password').value; var _URL  = '/wifi?ssid=' + _Name + '&password=' + _Pass; if(_Name == \"\"){ elert('Select wifi !'); return false; } else { SendURL(_URL);  }} </script>"; //ReloadWebTimer(500);
  return JVScript;
}
//////////////////////////////////////////////////////////////////////////////////
...
String CStyleCSS()
{
   String Button = "<style type='text/css'> .shadow{ text-shadow: 2px 2px 5px #000000;}  body{background: url(  ) fixed;}\
                    .bogoc{ border-radius: 25px;} .textinput{width:100%; height:50px;font-size:30px} .button{background: #73AD21; font-size:60px; text-align: center; }\
                    .tieude{color:blue;font-size:40px} .radio{width:30px; height:30px;} .text{color:white;font-size:25px} .TimerDown{color:blue;font-size:80px} .ComBoBox{width:200px; font-size:60px; }\
                    .myButton {-moz-box-shadow:inset 0px 39px 0px -24px #e67a73;-webkit-box-shadow:inset 0px 39px 0px -24px #e67a73;box-shadow:inset 0px 39px 0px -24px #e67a73;background-color:#e4685d;-moz-border-radius:4px;-webkit-border-radius:4px;border-radius:4px;border:1px solid #ffffff;display:inline-block;cursor:pointer;color:#ffffff;font-family:Arial;font-size:25px;padding:5px 15px;text-decoration:none;text-shadow:0px 1px 0px #b23e35;}\
...
void SearchWifi()
{
  ...
  ///////////////
  String HTML  = Header + JScriptAdmins() + JScriptTrangWifiConnect() + CStyleCSS() + TieuDe("&nbsp; <br> Wifi connect") ;
...
void Setting()
{
  ...
  String HTML  = Header +  JScriptAdmins() + JScriptTrangSetting()+ CStyleCSS();
         ...
         HTML += TieuDe("Admin");
         HTML += "<text class='text shadow' style='color:red'> Name </text><br>";
         HTML += "<p><input type='text' class='textinput' id='AdmName'  value='admin'></p>";
         HTML += "<text class='text shadow' style='color:red'> Password </text><br>";
         HTML += "<p><input type='text' class='textinput' id='AdmPass'  value='admin'></p> </div>";
...
void loop() 
{
  ...
  ////////////
  if(TTCongTac == "ESP_RESTART")     {  Restart(); }
  if(TTCongTac == "ESP_RESETDEFAULT"){  ResetDefault();  Restart(); }
  String SDau = TTCongTac.substring(0, 6);
  String SSau = TTCongTac.substring(6, TTCongTac.length());
  if(SDau == "WName:"){ Wifi_LuuName( SSau ); TTCongTac = ""; TTCongTac = ""; }
  if(SDau == "WPass:"){ Wifi_LuuPass( SSau ); TTCongTac = ""; TTCongTac = ""; }
  ////////////////////////////
  if( WiFi.status() == WL_CONNECTED )
  {
        ...
        ////////////
        String Webcode = LayChuoiJson(TTCongTac);
        if(DeviceUpdate == 0){ Serial.println(Webcode); }
        ////////////
    ...
    DeviceUpdate = 0;
    //TTCongTac    = "";
    TTCongTac    = "0,1,0,1&utsdvupdate=0" ;
    delay(1000);
   //////////////////////////////
}
