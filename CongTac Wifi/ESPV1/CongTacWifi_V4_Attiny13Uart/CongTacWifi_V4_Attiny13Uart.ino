...
String JScriptTrangHome()
{
    String JVScript = "<script>\
                          function TrangThaiNutBam(TenNut){ var TT = document.getElementById(TenNut).value ;  \
                          if(TT == 'On')  { document.getElementById(TenNut).style.background='#c0c0c0';   document.getElementById(TenNut).value = 'Off';  return 0; }   \
                          if(TT == 'Off') { document.getElementById(TenNut).style.background='#ff0000';   document.getElementById(TenNut).value = 'On' ;  return 1; }}  \
                          function NutBam(ThuTu, _Name){\
                           var TTNut =  TrangThaiNutBam(_Name); \                          
                           var url   = '/home?ThuTuCT=' + ThuTu + '&TrangThai=' + TTNut ; \
                           SendURL(url); \
                      }\
                     </script>";
  return JVScript;
...
String JScriptAdmins()
{
    String JVScript = "<script type='application/javascript'> function SendURL(URL){ var xhttp = new XMLHttpRequest(); xhttp.open('GET', URL, true); xhttp.send();}\
                     ...
                     function HRestart(){ SendURL('/home?restart=ok'); }\
...
String GiaoDienNutBam()
{
  ...
  String MauNen[2] = {"style='background-color:#c0c0c0c0'", "style='background-color:#ff0000'"};
  
  for(uint8_t i=0; i<4; i++)
  {
      uint8_t ViTri = CongTac[i];
      ThuTu = (String)( i + 1);
      HTML += "<tr><td > <a class='text' style='font-size:20px'> Cong tac " + ThuTu + "</a> </td>   <td > <input type='button' class='myButton' " + MauNen[ViTri] + " value='" +  OnOff[ViTri] + "'  ID='Nut" + ThuTu + "'  onclick=\"NutBam(" + ThuTu + ", 'Nut" + ThuTu + "')\"> </td></tr> ";
  }
   HTML  =  "<div align=center> <table border='0' cellspacing='5'>" + HTML  + "</table> </div> ";
   return HTML;
//  
//  String HTML =  "<div align=center>\
//                          <table border='0' cellspacing='5'>\
//                              <tr>\
//                                  <td > <a class='text' style='font-size:20px'> Cong tac 1 </a> </td>\
//                                  <td > <input type='button' class='myButton' value='Off'  ID='Nut1'  onclick=\"NutBam(1, 'Nut1')\"> </td>\
//                              </tr>\
//                              <tr>\
//                                  <td > <a class='text' style='font-size:20px'> Cong tac 2 </a> </td>\
//                                  <td > <input type='button' class='myButton' value='Off'  ID='Nut2'  onclick=\"NutBam(2, 'Nut2')\"> </td>\
//                              </tr>\
//                              <tr>\
//                                  <td > <a class='text' style='font-size:20px'> Cong tac 3 </a> </td>\
//                                  <td > <input type='button' class='myButton' value='Off'  ID='Nut3'  onclick=\"NutBam(3, 'Nut3')\"> </td>\
//                              </tr>\
//                              <tr>\
//                                  <td > <a class='text' style='font-size:20px'> Cong tac  4 </a> </td>\
//                                  <td > <input type='button' class='myButton' value='Off'  ID='Nut4'  onclick=\"NutBam(4, 'Nut4')\"> </td>\
//                              </tr>\ 
//                              <tr>\
//                                  <td > <a class='text' style='font-size:20px'> Restart </a> </td>\
//                                  <td > <input type='button' class='myButton' value='Restart'  ID='Restart'  onclick=\"HRestart()\"> </td>\
//                              </tr>\ 
//                          </table>\
//                      </div>
}
...
void TrangHome()
{
    ...
    if (server.hasArg("ThuTuCT"))  {  ThuTu     = server.arg("ThuTuCT").c_str();   }
    if (server.hasArg("TrangThai")){  TrangThai = server.arg("TrangThai").c_str(); }
    if(ThuTu !="")
    {
      uint8_t ThuTuNut  = ThuTu.toInt() - 1;
      uint8_t TThaiNut  = TrangThai.toInt();
      CongTac[ThuTuNut] = TThaiNut ;
      DeviceUpdate      = 1;
    }
    //////////////////
    CallRestart();
...
{
        HTTPClient http;
        String TTCongTac = (String)CongTac[0] + "," + (String)CongTac[1] + "," + (String)CongTac[2] + "," + (String)CongTac[3] ;
        String payload   = "";
        String SVLink    = "http://fablabs.store/cttmm/inc/onoff/?UTSID=" + SVID + "&utsvalue=" + TTCongTac + "&utstype=utsdevice&utsdvnum=" + DVNUM + "&utsdvupdate=" + DeviceUpdate;
...
//////////////////////////////////////////////////////////////////////////////////
void SuLyDuLieuNhan(String DuLieu)
{
   String CNhanDang  = DuLieu.substring(1,3);
   if(CNhanDang == "CT")
   {
       CongTac[0]  = DuLieu.substring(5,6).toInt();
       CongTac[1]  = DuLieu.substring(7,8).toInt();
       CongTac[2]  = DuLieu.substring(9,10).toInt();
       CongTac[3]  = DuLieu.substring(11,12).toInt();
   }
}
...
SoftwareSerial Attiny13Uart(14, 16); // RX, TX

////////////////////////////////////////////////////////////////////////////////////
void setup() {
  EEPROM.begin(512);           delay(100);
  Serial.begin(9600);          delay(100);
  Attiny13Uart.begin(115200);  delay(100);
  
  LoadThongTinCaiDat();
  ////////////////////////////////////////
  pinMode(LedTH, OUTPUT);
...
void loop() 
{
  ThoiGian = millis()/1000;
  server.handleClient();
  ////////////////////////////
  while (Attiny13Uart.available() > 0 ) 
  {
      char KyTu = Attiny13Uart.read();  uint8_t nbKyTu = (uint8_t)KyTu;  
      if( 31<nbKyTu && nbKyTu < 127 ){ ChuoiData+= KyTu; SoLanNhan++; } delay(5);
  }
  //Attiny13Uart.println("ABC");
  if(ChuoiData != "") { Attiny13Uart.print(ChuoiData);  ChuoiData = ""; }
  if(SoLanNhan >= 4) { Attiny13Uart.println(""); Attiny13Uart.print("-----: "); Attiny13Uart.print(SoLanNhan); Attiny13Uart.print(" :-----"); Attiny13Uart.println(""); SoLanNhan = 0; }
  ////////////////////////////
  digitalWrite(LedTH, HIGH); // Wifi Off led  stas
  if( WiFi.status() == WL_CONNECTED )
  {
        digitalWrite(LedTH, LOW);  // Wifi On led  stas
        ////////////
        ...
        if((ThoiGian%3) == 0)
        {
           String  OnlineData = LayChuoiJson();
           if(OnlineData != "")
           {
              SuLyDuLieuNhan( OnlineData );
