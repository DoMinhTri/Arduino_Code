...
IPAddress ip(192, 168, 1, 123); // IP address, may need to change depending on network
        ...
        while (client.connected()) {
            if (client.available()) {   // client data available to read
                ...
                if (c == '\n' && currentLineIsBlank) {
                    ...
                    if (StrContains(HTTP_req, "GET / ") || StrContains(HTTP_req, "GET /rec.php")) 
                    {
                        client.println("HTTP/1.1 200 OK");
                        client.println("Content-Type: text/html");
                        client.println("Connnection: close");
                        client.println();
                        webFile = SD.open("rec.php");        // open web page file
                    }
                    else if (StrContains(HTTP_req, "GET /page2.htm")) 
                    {
                        ...
                        client.println("Connnection: close");
                        client.println();
                        webFile = SD.open("/1/page2.htm");        // open web page file
                    }
                    else if (StrContains(HTTP_req, "GET /pic.jpg")) {
                        webFile = SD.open("/1/pic.jpg");
                        if (webFile) {
                            client.println("HTTP/1.1 200 OK");
                            client.println();
                        }
