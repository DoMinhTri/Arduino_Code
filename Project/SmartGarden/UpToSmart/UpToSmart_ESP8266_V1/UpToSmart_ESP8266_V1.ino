...
{
    /*
    if(ChuoiData.length()> 61)
    {
      if(ChuoiData.substring(0,1) == "U")
      {
         ChuoiDuLieuMCU = ChuoiData.substring(0,62);
      }
    }
    */
    ChuoiDuLieuMCU = ChuoiData;// ChuoiData.substring(0,62);
    /*
...
char* JScriptSuLyChung()
{
    char* JVScript = "<script type='application/javascript'> function SendURL(URL){ var xhttp = new XMLHttpRequest(); xhttp.open('GET', URL, true); xhttp.send();}\
                          function AnNut3Nut(){ document.getElementById('Van1').style.visibility = 'hidden';  document.getElementById('Van2').style.visibility = 'hidden';  document.getElementById('Van3').style.visibility = 'hidden';  setTimeout('Show3Nut()',1000); } \
                          function Show3Nut(){ document.getElementById('Van1').style.visibility = 'visible';  document.getElementById('Van2').style.visibility = 'visible'; document.getElementById('Van3').style.visibility = 'visible'; }\
                          function OpenLink(URL){ window.location.href = URL ; } function ReloadWeb(){ window.location.reload(); } function ReloadWebTimer(_timer){ setTimeout('ReloadWeb()',_timer); }</script>";
  return JVScript;
}
////////////////////////////////////////////////////////////////////////////////// Script Access point
String JScriptAdmins()
{
    String JVScript = "<script> function WRestart(){ SendURL('/wifi?restart=ok'); }\
...
{
    String JVScript = "<script> var nTimerdown = 0;\
                          function VanConTrol( _Name1, _Name2){  var GTri = document.getElementById(_Name2).value; if(GTri == 1){ document.getElementById(_Name1).value = 'Off'; document.getElementById(_Name2).value = 0;} else { document.getElementById(_Name1).value = 'On';  document.getElementById(_Name2).value = 1;} TuoiUuTien(); AnNut3Nut(); }\
                          function TuoiUuTien(){ nTimerdown = -1; var GTri = document.getElementById('CbbTuoiUuTien').value; setTimeout('KiemTraTuoiUuTien( ' + GTri + ' )',1000);  var sGT1 = document.getElementById('GT1').value ; var sGT2 = document.getElementById('GT2').value ; var sGT3 = document.getElementById('GT3').value ; var url = '?RelayTimer=' + sGT1 + '\,' + sGT2 + '\,' + sGT3 + '\,' + GTri ; if(GTri > 0) { SendURL(url); } else{ SendURL('?RelayTimer=0,0,0,0,0'); }}\
                          function KiemTraTuoiUuTien(nTime){ nTimerdown = nTime*60; ThoiGianLui(); }\
                          function ThoiGianLui(){ if(nTimerdown>= 0){ var phut = parseInt(nTimerdown/60); var giay = nTimerdown%60; document.getElementById('TimerDown').innerHTML = phut + ':' + giay ; nTimerdown--; var sVal = 'ThoiGianLui('+nTimerdown+')'; setTimeout(sVal,1000); }} </script>";
  return JVScript;
}
...
String JScriptTrangSetting()
{
  ...
  String JVScript = "<script> function AnNutSave()  { document.getElementById('btSave').style.visibility = 'hidden'; }   function HienNutSave(){ document.getElementById('btSave').style.visibility = 'visible'; }\
                        ...
                        if(aname == '') { alert('Please input access point name !'); return false; } if(NameCheck('APPass', 8, 'Access point password 8 character or more !') == false) { return false; }\
                        if(IPCheck('APIP', 'Access point IP error !') == false)  { return false; }   if(NameCheck('DName', 5, 'Admin name 5 character or more !') == false) { return false; } if(NameCheck('DPass', 5, 'Admin password 5 character or more !') == false) { return false; }\
                        if(NameCheck('SVID', 16, 'ID need 16 character !') == false) { return false; }  if(IPCheck('WIP', 'Wifi IP error !') == false)  { return false; } if(wport == '') { alert('Please input wifi port !'); return false; }\
...
String ComBoxTuoiNhanh()
{
  ...
  String cbb = "<select size='1' class='ComBoBox' id='CbbTuoiUuTien' onchange='TuoiUuTien()'>";
                ...
                { 
                      if( i< 10)
                      {
                            if( i == 1){ chon = "selected"; } else { chon = ""; }
                            cbb += "<option " + chon + " value='0" + (String)i + "'>" + (String)i+ "</option>"; 
                      }
                      else{ cbb += "<option value='"+ (String)i+ "'>"+ (String)i+ "</option>"; }
                }
                cbb += "</select>";
  return cbb;
}
...
{
  String HTML = "<div align=center> <table border='0' cellspacing='5' width='100%'>\
                      <tr> <td align='left'> <a class='text'> Line 1 </a> </td>\
                           <td align='right'> <input type='button' class='myButton' value='Off'  ID='Van1'  onclick=\"VanConTrol('Van1','GT1')\"> <input type='hidden' value='0' id='GT1'></td> </tr>\
                      <tr> <td align='left'> <a class='text'> Line 2 </a> </td>\
                           <td align='right'> <input type='button' class='myButton' value='Off'  ID='Van2'  onclick=\"VanConTrol('Van2','GT2')\"> <input type='hidden' value='0' id='GT2'></td> </tr>\
                      <tr> <td align='left'> <a class='text'> Line 3 </a> </td>\
                           <td align='right'> <input type='button' class='myButton' value='Off'  ID='Van3'  onclick=\"VanConTrol('Van3','GT3')\"> <input type='hidden' value='0' id='GT3'></td> </tr>\
                      <tr> <td align='left'> <a class='text'> Timer </a> </td> <td align='right'>";
         HTML += "<font class='TimerDown shadow' id='TimerDown'></font> &nbsp; &nbsp; " + ComBoxTuoiNhanh()+ "</td></tr></table></div>";
  return HTML;
}
//////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////
...
void Desktop() 
{
  if (server.hasArg("RelayTimer"))
  { 
    String sRelayTimer  = server.arg("RelayTimer").c_str();
    if(sRelayTimer != "") { Serial.println("UTS_TUOIUUTIEN:" + sRelayTimer); }
    else if(sRelayTimer != "0"){ Serial.println("UTS_RESTART"); }
  }
...
void SearchWifi()
{
  ...
  {    
      ...
      if(WifiName != "")
      {
          ...
          ///////////////
          delay(5000);
          if(WiFi.status()== WL_CONNECTED){  Wifi_LuuIP(LayChuoiIP(WiFi.localIP())); Restart(); }
      }
  }
  ///////////////
...
void SuLyGiaTriChuoiNhan(String ChuoiNhan)
{
  ...
  if(ChuoiNhanDang == "UTS_ID")      { SV_LuuSVID(ChuoiGiaTri);      }
  ...
  if(ChuoiNhanDang == "UTS_RESTART") { Restart();                    }
  if(ChuoiNhanDang == "UTS_RESET")   { ResetDefault();  Restart();   }
}
//////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////
...
void setup() 
{
  ...
  */
  ...
  server.begin();
  delay(2500);
  Serial.println("UTS_RESTART:1"); // Restart MCU
  delay(2000);
}
//////////////////////////////////////////////////////////////////////////////////
String DataUART = "";
...
void loop() 
{
   ...
   ////////////////////////////
   /*
   if((WiFi.status() == WL_CONNECTED) && (ChuoiDuLieuMCU != ""))
   {
        HTTPClient http;
        String SVLink = "http://uptosmart.vn/homegreen/data/?utsid=" + SVID + "&utsvalue=" + ChuoiDuLieuMCU;
