...
void KiemTraTuoiCay()
{
    if(nLanTuoi >0)
    {
       for(uint8_t i = 0; i< nLanTuoi; i++)
       {
           ...
           uint8_t ThuTuRelay_Van = cTTRelay - 1;
           //////////////////
           switch (cKieuTuoi)
          {
              case 1:
              {
                  Relay[ThuTuRelay_Van] = KiemTraGioTuoi(cGioTuoi, cPhutTuoi, cTGTuoi);
                  break;
              }
              case 2:
              {
                  if(DoAmDat[ThuTuRelay_Van] < cDoAmMax)
                  {
                      Relay[ThuTuRelay_Van] = KiemTraGioTuoi(cGioTuoi, cPhutTuoi, cTGTuoi);
                  }
                  break;
...
//////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////
// {"Name":"UpToSmart.VN","PumpPort":"0,0,0,0","Temperature":"34","Sensor":"00,00,00","Weather":"00,30,0","NumOfTimer":"00","Date":"08/01/00/10/25/47"}
// {Name:"UpToSmart.VN",PumpPort:"0,0,0,0",Tem:"29",SenS:"00,00,00",Wea:"00,30,0",NumOfTimer:"00",Date:"21/02/00/12/09/01"}

//void ShowThongTinMoiTruong()
//{
//    DataSensor  = "UTS_MCU:" + (String)Relay[0]   + "," + (String)Relay[1] + "," + (String)Relay[2]  + "," +  nNhietDo + ",";
//    DataSensor += ChuSo2DonVi(DoAmDat[0]) + "," + ChuSo2DonVi(DoAmDat[1]) + "," + ChuSo2DonVi(DoAmDat[2]) + ","  + (String)TroiMua + ",";
//    DataSensor += ChuSo2DonVi(ngay) + "," + ChuSo2DonVi(thang) + "," + ChuSo2DonVi(nam) + "," + ChuSo2DonVi(gio) + "," + ChuSo2DonVi(phut)+ "," + ChuSo2DonVi(giay)   ;     
...
void ShowThongTinMoiTruong()
{
    DataSensor  = "UTS_MCU:{Name:\"UpToSmart.VN\",PumpPort:\"" + (String)Relay[0]   + "," + (String)Relay[1] + "," + (String)Relay[2] + "," + (String)Relay[3] + "\",Tem:\"" +  nNhietDo           + "\",";
    DataSensor += "SenS:\"" + ChuSo2DonVi(DoAmDat[0]) + "," + ChuSo2DonVi(DoAmDat[1]) + "," + ChuSo2DonVi(DoAmDat[2])  + "\",Wea:\"" + ChuSo2DonVi(DoAmMua)   + "," + ChuSo2DonVi(nThoiTiet)  + ","  + (String)TroiMua + "\",NumOfTimer:\""  + ChuSo2DonVi(nLanTuoi) + "\",";               
    DataSensor += "Date:\"" + ChuSo2DonVi(ngay) + "/" + ChuSo2DonVi(thang) + "/" + ChuSo2DonVi(nam) + "/" + ChuSo2DonVi(gio) + "/" + ChuSo2DonVi(phut)+ "/" + ChuSo2DonVi(giay) + "\"}";     
    Serial.println(DataSensor);
    DataSensor  = "";
}
//////////////////////////////////////////////////////////////////////////////////
