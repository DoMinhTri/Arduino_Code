...
delay(300);
  ...
  //  listDir(SD, "/", 0);
   // removeDir(SD, "/mydir");
    listDir(SD, "/", 2);
//    writeFile(SD, "/hello.txt", "esp32-cam ");
...
void loop() {
 ...
 Serial.println("mainloop :manual Camera gelukt  ");
      if (testSDkaart ) {
       ...
       writeFile(SD, "/loopfBUF.txt", (char *)foto->buf);// does not work 
          writeFile(SD, "/loopfotoLEN.txt", (char *)foto->buf);//  
